...
export const ProviderServiceFormImage: React.FC<Props> = ({ formik }) => {
    ...
    <>
      <Flex gap={8} wrap>
        <ImgCrop aspect={1} onModalOk={onModalOk}>
          <Upload maxCount={1} showUploadList={false} disabled={formik.isSubmitting}>
            <AppButton
              icon={formik.values.image ? <RedoOutlined /> : <UploadOutlined />}
      ...
      {previewImage && (
        <div className='mt-6 flex justify-center'>
          <Image className='max-w-80' alt='Profile picture' preview={false} src={previewImage} />
        </div>
      )}
    </>
  )
