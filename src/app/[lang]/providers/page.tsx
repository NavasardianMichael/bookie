...
import { buildExploreHref, parseExploreParams, RawSearchParams } from './exploreParams'
import { ExplorePendingProvider, ProvidersResultsOverlay } from './ExplorePending'
    ...
    <ExplorePendingProvider>
      <PageShell className='flex flex-col gap-10'>
        ...
        {/* Heading + toolbar stay outside Suspense so a new query does not remount the
            sort/filter sheet or jump the title. The boundary has no key: a key would
            throw the current grid away for a skeleton, and the overlay is what covers
            the wait instead. */}
        <Section title={t('providersTitle')} className='gap-8' actions={<ProviderExploreToolbar params={params} />}>
          <ProvidersResultsOverlay>
            <Suspense fallback={<ProvidersResultsSkeleton />}>
